...
import type { QuizRunnerCopy } from "@/lib/quizTranslations";
export function QuizRunner({questions, copy: t}: {questions: QuizQuestion[]; copy: QuizRunnerCopy}) {
  ...
  return <div style={{maxWidth: 860, margin: "4rem auto", padding: "1.5rem"}}>{q ? <>
    <p>{t.progress[index]}</p><h1 dangerouslySetInnerHTML={{__html: q.question}} />
  ...
  </> : <><h1>{t.scoreTitle}</h1><p>{t.scores[score]}</p><button onClick={() => {setIndex(0); setAnswers({});}}>{t.restart}</button></>}</div>;
}
